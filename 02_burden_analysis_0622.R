...
if (is.null(loaded_names)) {
  fit6 <- readRDS(fit6_path)
} else if (!exists("fit6", inherits = FALSE)) {
  if (length(loaded_names) == 1) {
    fit6 <- get(loaded_names[1])
  } else {
    stop("The fit file loaded, but no object named 'fit6' was found: ", fit6_path)
  }
}

cat("Loaded calibrated fit from:", fit6_path, "\n")
...
posterior_samples_list <- rstan::extract(fit6, pars = mcmc_param_names, permuted = TRUE)
...
posterior_summary <- posterior_samples_mcmc_selected %>%
  summarise(across(
    everything(),
    list(
...
write.csv(posterior_summary, posterior_summary_csv, row.names = FALSE)

posterior_audit <- data.frame(
  check = "posterior_samples_used_were_extracted_from_fit6",
  fit6_path = fit6_path,
  n_rows = nrow(posterior_samples_mcmc_selected),
  n_columns = ncol(posterior_samples_mcmc_selected),
  parameter_names = paste(colnames(posterior_samples_mcmc_selected), collapse = "; "),
  recorded_rds = posterior_full_rds,
  recorded_csv = posterior_full_csv,
  recorded_summary_csv = posterior_summary_csv,
  rds_md5 = as.character(tools::md5sum(posterior_full_rds)),
  csv_md5 = as.character(tools::md5sum(posterior_full_csv)),
  created_at = as.character(Sys.time()),
  stringsAsFactors = FALSE
)
write.csv(posterior_audit, posterior_audit_csv, row.names = FALSE)

cat("Saved full posterior samples extracted from fit6 to:", posterior_full_rds, "\n")
cat("Saved posterior extraction audit to:", posterior_audit_csv, "\n")
...
# --- 5. Function to Calculate Annual Outcomes ---
...
posterior_draw_indices <- sample(
  seq_len(nrow(posterior_samples_mcmc_selected)),
  size = n_psa_samples,
  replace = TRUE
)
...
write.csv(
  data.frame(
    sample_id = seq_len(n_psa_samples),
    posterior_row = posterior_draw_indices
  ),
  posterior_index_file,
  row.names = FALSE
)

posterior_samples_used_in_psa <- bind_cols(
  data.frame(
    sample_id = seq_len(n_psa_samples),
    posterior_row = posterior_draw_indices
  ),
  posterior_samples_mcmc_selected[posterior_draw_indices, , drop = FALSE]
)
saveRDS(posterior_samples_used_in_psa, posterior_psa_used_rds)
write.csv(posterior_samples_used_in_psa, posterior_psa_used_csv, row.names = FALSE)

cat("\nRunning PSA with", n_psa_samples, "samples...\n")
cat("MCMC posterior has", nrow(posterior_samples_mcmc_selected), "samples available.\n")
cat("PSA seed:", psa_seed, "\n")
cat("Saved fixed posterior draw indices to:", posterior_index_file, "\n")
cat("Saved exact posterior samples used in PSA to:", posterior_psa_used_rds, "\n")
...
psa_results_list <- foreach(i = 1:n_psa_samples, .packages = c("deSolve", "dplyr")) %dopar% {
  ...
  # 1. Use pre-sampled posterior row index for exact PSA reproducibility.
  random_idx <- posterior_draw_indices[i]
...
# --- 8. Final, Detailed Analysis & Visualization (with Overall & Faceted Views) ---
...
# --- 8a. Prepare Detailed Annual Data (No Changes Here) ---

# Filter data for each scenario separately
baseline_clean <- psa_annual_results %>% 
  filter(scenario == "baseline") %>%
  select(sample_id, year, 
         sus_deaths_h_base = annual_sus_deaths_h, res_deaths_h_base = annual_res_deaths_h,
         sus_deaths_c_base = annual_sus_deaths_c, res_deaths_c_base = annual_res_deaths_c)

no_res_clean <- psa_annual_results %>%
  filter(scenario == "no_resistance") %>%
  select(sample_id, year, 
         sus_deaths_h_nores = annual_sus_deaths_h, res_deaths_h_nores = annual_res_deaths_h,
         sus_deaths_c_nores = annual_sus_deaths_c, res_deaths_c_nores = annual_res_deaths_c)

# Join the two clean datasets for a direct comparison
annual_burden_comparison <- inner_join(
  baseline_clean,
  no_res_clean,
  by = c("sample_id", "year")
)

# --- 8b. Calculate Burden Components AND Overall Burden per Sample ---

# This data frame calculates the burden for EACH of the 1000 PSA runs.
# It is the source for all our summary statistics.
burden_per_sample <- annual_burden_comparison %>%
  mutate(
    # Hospital components
    direct_burden_h = res_deaths_h_base,
    indirect_burden_h = sus_deaths_h_base - sus_deaths_h_nores,
    total_incremental_h = direct_burden_h + indirect_burden_h,
    
    # Community components
    direct_burden_c = res_deaths_c_base,
    indirect_burden_c = sus_deaths_c_base - sus_deaths_c_nores,
    total_incremental_c = direct_burden_c + indirect_burden_c,
    
    # NEW: Overall (Total) components
    direct_burden_overall = direct_burden_h + direct_burden_c,
    indirect_burden_overall = indirect_burden_h + indirect_burden_c,
    total_incremental_overall = total_incremental_h + total_incremental_c
  )


# --- 8c. Create the Summary Table with Overall and Component CIs ---
# Now we summarize the per-sample data to get means and CIs
final_summary_table <- burden_per_sample %>%
  group_by(year) %>%
  summarise(
    # Summarize Overall Burden
    mean_total_overall = mean(total_incremental_overall, na.rm = TRUE),
    lower_total_overall = quantile(total_incremental_overall, 0.025, na.rm = TRUE),
    upper_total_overall = quantile(total_incremental_overall, 0.975, na.rm = TRUE),
    
    # Summarize Hospital Burden
    mean_total_h = mean(total_incremental_h, na.rm = TRUE),
    lower_total_h = quantile(total_incremental_h, 0.025, na.rm = TRUE),
    upper_total_h = quantile(total_incremental_h, 0.975, na.rm = TRUE),
    
    # Summarize Community Burden
    mean_total_c = mean(total_incremental_c, na.rm = TRUE),
    lower_total_c = quantile(total_incremental_c, 0.025, na.rm = TRUE),
    upper_total_c = quantile(total_incremental_c, 0.975, na.rm = TRUE),
    
    .groups = 'drop'
  ) %>%
  mutate(year_num = as.numeric(gsub("year_", "", year))) %>%
  arrange(year_num)

# --- 8d. Print the Overall Annual Summary ---
cat("\n\n=== OVERALL ANNUAL AVERTED DEATHS (Hospital + Community) ===\n\n")
for (i in 1:nrow(final_summary_table)) {
  cat(sprintf("--- Year %d Net Total Averted: %.0f (95%% CI: %.0f to %.0f) ---\n",
              final_summary_table$year_num[i],
              final_summary_table$mean_total_overall[i],
              final_summary_table$lower_total_overall[i],
              final_summary_table$upper_total_overall[i]))
}

# --- 8e. Visualize the Overall Burden ---
plot_overall_burden <- ggplot(final_summary_table, aes(x = year_num)) +
  geom_ribbon(aes(ymin = lower_total_overall, ymax = upper_total_overall), fill = "red", alpha = 0.3) +
  geom_line(aes(y = mean_total_overall), color = "red", size = 1.2) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
  labs(
    title = "Overall Net Averted Deaths by Eliminating Resistance",
    subtitle = "Total burden from Hospital and Community combined, with 95% credible interval",
    x = "Year of Economic Horizon",
    y = "Net Annual Averted Deaths (Overall)"
  ) +
  theme_bw()

print(plot_overall_burden)

# --- 8f. Visualize the Faceted (Hospital vs. Community) Burden ---

# First, prepare the data for faceting by making it "longer"
faceted_plot_data <- final_summary_table %>%
  select(year_num, ends_with("_h"), ends_with("_c")) %>%
  pivot_longer(
    cols = -year_num,
    names_to = c(".value", "origin"), # ".value" tells pivot_longer to create columns from the first part of the name
    names_pattern = "(mean_total|lower_total|upper_total)_(h|c)"
  ) %>%
  mutate(origin = ifelse(origin == "h", "Hospital", "Community"))

# Now create the faceted plot
plot_faceted_burden <- ggplot(faceted_plot_data, aes(x = year_num, y = mean_total, color = origin)) +
  geom_ribbon(aes(ymin = lower_total, ymax = upper_total, fill = origin), alpha = 0.3, show.legend = FALSE) +
  geom_line(size = 1.2, show.legend = FALSE) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
  
  # Use facets to create separate panels
  facet_wrap(~origin, scales = "free_y", labeller = label_both) +
  
  scale_color_manual(values = c("Hospital" = "#0072B2", "Community" = "#009E73")) +
  scale_fill_manual(values = c("Hospital" = "#56B4E9", "Community" = "#a1d99b")) +
  
  labs(
    title = "Net Averted Deaths by Origin",
    subtitle = "Positive values indicate a health benefit from eliminating resistance",
    x = "Year of Economic Horizon",
    y = "Net Annual Averted Deaths"
  ) +
  theme_bw() +
  theme(strip.background = element_rect(fill="grey90"),
        strip.text = element_text(face = "bold"))

print(plot_faceted_burden)
...
color_overall <- "#2F2F2F"
color_hospital <- "#0072B2"
color_community <- "#D55E00"

# --- 2. Determine a Unified Y-Axis Scale ---
min_y_limit <- min(final_summary_table$lower_total_overall, 
                   final_summary_table$lower_total_h, 
                   final_summary_table$lower_total_c)

max_y_limit <- max(final_summary_table$upper_total_overall, 
                   final_summary_table$upper_total_h, 
                   final_summary_table$upper_total_c)

# --- 3. Create Three Separate Plots with Modified Labels ---

# Plot 1: Overall (Leftmost plot gets the Y-axis label)
plot_overall <- ggplot(final_summary_table, aes(x = year_num)) +
  geom_ribbon(aes(ymin = lower_total_overall, ymax = upper_total_overall), fill = color_overall, alpha = 0.2) +
  geom_line(aes(y = mean_total_overall), color = color_overall, linewidth = 1.2) +
  coord_cartesian(ylim = c(min_y_limit, max_y_limit)) +
  scale_y_continuous(labels = label_comma()) +
  labs(
    title = "Overall", 
    x = NULL, # No x-axis label
    y = "Annual Deaths Attributable to Resistance" # The shared y-axis label
  ) +
  theme_bw()

# Plot 2: Hospital (Middle plot has no axis labels)
plot_hospital <- ggplot(final_summary_table, aes(x = year_num)) +
  geom_ribbon(aes(ymin = lower_total_h, ymax = upper_total_h), fill = color_hospital, alpha = 0.2) +
  geom_line(aes(y = mean_total_h), color = color_hospital, linewidth = 1.2) +
  coord_cartesian(ylim = c(min_y_limit, max_y_limit)) +
  scale_y_continuous(labels = label_comma()) +
  labs(
    title = "Hospital", 
    x = "Year", # The shared x-axis label
    y = NULL # No y-axis label
  ) +
  theme_bw()

# Plot 3: Community (Rightmost plot has no axis labels)
plot_community <- ggplot(final_summary_table, aes(x = year_num)) +
  geom_ribbon(aes(ymin = lower_total_c, ymax = upper_total_c), fill = color_community, alpha = 0.2) +
  geom_line(aes(y = mean_total_c), color = color_community, linewidth = 1.2) +
  coord_cartesian(ylim = c(min_y_limit, max_y_limit)) +
  scale_y_continuous(labels = label_comma()) +
  labs(
    title = "Community", 
...
# --- 4. Combine the Plots ---
# Patchwork automatically aligns the plots and their axes.
final_panel_plot <- (plot_overall | plot_hospital | plot_community) +
  plot_annotation(
    title = "Annual Deaths Attributable to Resistance",
    subtitle = "Comparison across overall, hospital, and community settings"
  )
